...
describe('Contract: KingAutomaton', () => {
  const KingAutomaton = artifacts.require("KingAutomaton");

  describe('Exchange', async (accounts) => {
    beforeEach(async () => {
    ...
    it('complete test buy() and sellNow()', async () => {
      ...
      assert.equal(await koh.getExchangeBalance(), 0, "There should be no AUTO left in the exchange account");
    ...
    it('complete test sell() and buyNow()', async () => {
      ...
      exchangeBalance = await koh.getExchangeBalance();
      assert.equal(exchangeBalance.toString(), minOrderAUTO.toString(),
          "The AUTO should be deposited in the exchange special purpose account");
    ...
    it('buy() should create and register order correctly', async () => {
      ...
      assert.equal(await web3.eth.getBalance(koh.address), minOrderETH.toString(), "Incorrect ETH balance in contract after buy()");
    ...
    it('sell() should create and register order correctly', async () => {
      await koh.sell(minOrderAUTO, minOrderETH);
      assert.equal((await koh.getExchangeBalance()).toString(), minOrderAUTO.toString(), "Incorrect AUTO balance in exchange after sell()");
      assert.equal(await koh.getOrdersLength(), 1, "Order not created");
      let order = await koh.orders(0);
      assert.equal(order.AUTO.toString(), minOrderAUTO.toString(), "Invalid AUTO in order");
      assert.equal(order.ETH.toString(), minOrderETH.toString(), "Invalid ETH in order");
    ...
    it('cancel() after a buy() should return ETH back to owner', async () => {
      ...
      assert.equal(finalETHBalance.toString(), expectedETHBalance.toString(), "ETH should have been returned (minus gas costs)");
    });
    ...
    it('cancel() after a sell() should return AUTO back to owner', async () => {
      ...
      assert.equal(finalETHBalance.toString(), expectedETHBalance.toString(), "Should not have changed ETH balance except for gas costs");
