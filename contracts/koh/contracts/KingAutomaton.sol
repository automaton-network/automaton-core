...
contract KingAutomaton {
  ...
  */
  ...
  function completeProposal(uint256 _id) private {
    updateProposalState(_id);
    require(ballotBoxes[_id].state == BallotBoxState.Active, "Ballot is not active!");
    Proposal storage p = proposals[_id];
  ...
  function withdraw(uint256 _value) external {
    require(_value <= withdraws[msg.sender]);
    withdraws[msg.sender] -= _value;
  ...
  function removeOrder(uint256 _id) private {
    Order memory null_order;
    null_order.AUTO = 0;
    null_order.ETH = 0;
    null_order.owner = address(0);
    null_order.orderType = OrderType.None;
    orders[_id] = null_order;
  }
  ...
  function sellNow(uint256 _id, uint256 _AUTO, uint256 _ETH) public {
    ...
    uint256 w = withdraws[msg.sender];
    require(w + _ETH > w);
    withdraws[msg.sender] += _ETH;
    removeOrder(_id);
  ...
  function sell(uint256 _AUTO, uint256 _ETH) public returns (uint256 _id){
    ...
    transfer(DEXAddress, _AUTO);
     _id = ++ids;
    orders[_id] = Order(_AUTO, _ETH, msg.sender, OrderType.Sell);
  }
  ...
  function buyNow(uint256 _id, uint256 _AUTO) public payable {
    ...
    uint256 w = withdraws[o.owner];
    require(w + msg.value > w);
    withdraws[o.owner] += msg.value;
    removeOrder(_id);
  ...
  function cancelOrder(uint256 _id) public {
    ...
    if (o.orderType == OrderType.Buy) {
      require(withdraws[msg.sender] + o.ETH > withdraws[msg.sender]);
      withdraws[msg.sender] += o.ETH;
