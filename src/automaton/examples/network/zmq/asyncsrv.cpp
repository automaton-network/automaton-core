...
class client_task {
 public:
    ...
    void start() {
        ...
        char identity[10] = {};
        sprintf(identity, "%04X-%04X", within(0x10000), within(0x10000)); // NOLINT
        printf("%s\n", identity);
        ...
        try {
            while (true) {
                ...
                sprintf(request_string, "request #%d", ++request_nbr); // NOLINT
...
class server_worker {
 public:
    server_worker(zmq::context_t &ctx, int sock_type) // NOLINT
        : ctx_(ctx),
