...
// TODO(kari): 1) Add support for fixed-point numbers. 2) Show address as string in hex?
...
static std::ostream& operator<<(std::ostream& os, const abi_type& obj) {
  ...
  if (obj & fixed_size_bytes) {
    os << "fixed_size_bytes ";
  }
  if (obj & dynamic_size_bytes) {
    os << "dynamic_size_bytes ";
  }
  if (obj & fixed_size_array) {
    os << "fixed_size_array ";
  }
  if (obj & dynamic_size_array) {
    os << "dynamic_size_array ";
  }
  return os;
}
...
static type get_type(std::string s) {
  ...
  if (k1 != std::string::npos) {
    auto k2 = s.find(']');
    if (k1 == (k2 - 1)) {
      t.s_type = (abi_type)(t.s_type | dynamic_size_array);
    } else {
      t.s_type = (abi_type)(t.s_type | fixed_size_array);
...
static uint32_t calculate_offset(std::vector<std::string> params) {
  ...
  for (uint32_t i = 0; i < params.size(); ++i) {
    ...
    if (t.s_type & fixed_size_array) {
      ...
      while (t.s_type & fixed_size_array) {
...
static std::string int_to_32(const std::string& sig, const std::string& s) {
  CryptoPP::Integer::Signedness sign = sig[0] == 'u' ? CryptoPP::Integer::UNSIGNED : CryptoPP::Integer::SIGNED;
...
static std::string _32_to_int(const std::string& sig, const std::string& s) {
  CryptoPP::Integer::Signedness sign = sig[0] == 'u' ? CryptoPP::Integer::UNSIGNED : CryptoPP::Integer::SIGNED;
  ...
  if (t.s_type & fixed_size_array) {
    ...
    *head_pos += 32;
    ...
    if (t.str == "bool") {
      ...
      encoded = uint_to_32(json_data.get<bool>() ? 1 : 0);
...
static std::string decode_param(type t, const std::string& data, uint64_t pos) {
  if (t.s_type & fixed_size_array) {
    ...
    std::stringstream ss;
    ss << '[';
    if (tp.s_type == numerical || tp.s_type == fixed_size_bytes) {
